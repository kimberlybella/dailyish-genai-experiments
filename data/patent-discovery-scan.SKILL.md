...
Data file (bash path): `/sessions/epic-nice-davinci/mnt/data/patents.ts`

Use `mcp__workspace__bash` to read and write this file — do NOT use the Read/Write/Edit file tools, which will fail in scheduled runs. Read the file first with:
```
cat /sessions/epic-nice-davinci/mnt/data/patents.ts
```
If that path does not exist, locate it with: find /sessions -name "patents.ts" 2>/dev/null

Extract every `id` already in the `patents` array. Do NOT add any patent whose `id` is already present.

To write new entries, use Python via bash to read the file as a string, insert the new entry objects at the correct position (sorted by `datePublished` descending within each category group), and write the full updated file back.
